...
##### column threadholds
th_col=0.005 # how much deviation from mean when identify spokes columns
...
for i in range(spokesnumb):
	...
	for k in range(len(spkbox_prebpx[i])):
		#index, value = max(enumerate(abs(spkbox_prebpx[i][k])), key=operator.itemgetter(1))
	...
	if len(spokes_ind_lon_prebpx[i])==0:
		sort_box=sorted(spkbox_prebpx[i].flatten())
		for k in range(len(spkbox_prebpx[i])):
			minar=np.where(spkbox_prebpx[i] == (sort_box[k]))
			#print(minar)
			spokes_ind_lon_prebpx[i].append(spkbox_ind_lon_prebpx[i][int(minar[1])])
			spokes_ind_rad_prebpx[i].append(spkbox_ind_rad_prebpx[i][int(minar[0])])
