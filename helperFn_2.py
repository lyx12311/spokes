...
def getdark(datapoints,spokesdata,darkpix):
	...
	data,index=zip(*sorted(zip(dataflat,range(len(dataflat)))))
...
# function to clean up very long rows based on the median
...
def findspoke_num(spokesdata,boundsiz,minrowsiz):
	...
	while bound in spokesdata:
		...
		rows_s=Counter(idel_a).keys()
		colums_s=Counter(idel_a).values()
